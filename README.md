...
ただしこれらはスクリプトによる検証であって、ユニットテストではありません。判定ロジック(在席状態の解決・氏名の正規化・会議室の占有判定)は純関数として切り出してありますが、テストコードはまだ入れていません。現時点の検証は上記のスクリプトと実機確認に依存しており、これは弱点として残しています。
...
| 検証 | 画面・データ・配布物の3ハーネス(ユニットテストは未導入) |
...
ユニットテストは未導入です。上記は型検査・静的解析・データ整合性の検証であり、テストコードによる検証ではありません。
