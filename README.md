...
最大の課題は、必要なデータが2つのオンプレミス製品に分かれていて、しかも互いにつながっていないことでした。予定を持つ Garoon は社内からしか参照できず、社外からも使える Pleasanter とは経路が別だったためです。

```mermaid
flowchart LR
  C["ブラウザ / iOS アプリ"] --> A["Akamai<br>リバースプロキシ"]
  A --> P["Pleasanter<br>オンプレ・社外公開"]
  P -->|"サーバー間 API"| G["Garoon<br>オンプレ・社内限定"]
```

そこで Akamai をリバースプロキシとして挟み、クライアントからは Pleasanter だけを見せる形にしました。Garoon へのリクエストは Pleasanter 側のサーバーから発行し、受け取った結果をクライアントへ返します。社内限定のリソースを社外へ開かずに、外出先からでも予定を参照できます。この経路の設計と実装も担当しました。
